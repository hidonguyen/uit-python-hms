...
class RoomTypeRepository:
    ...
    async def create(self, room_type_data: Dict[str, Any]) -> RoomType:
        """Tạo loại phòng mới."""
    ...
    async def update(self, room_type_id: int, room_type_data: Dict[str, Any]) -> Optional[RoomType]:
        """Cập nhật loại phòng."""
    ...
    async def delete(self, room_type_id: int) -> bool:
        ...
        if rooms_count.scalar() > 0:
            raise ValueError("Không thể xóa loại phòng vì vẫn còn phòng đang sử dụng")
        ...
        if bookings_count.scalar() > 0:
            raise ValueError("Không thể xóa loại phòng vì vẫn còn booking đang sử dụng")
